...
# Construct an Avro "Name" value for a given type, name and namespace
avro_make_type_name <- function(type,name=NULL,namespace=NULL){
  ifelse(type %in% c("record","fixed","enum"),
...
# Converts a factor level value into an ravro-defined symbol value
# vectorized
ravro_make_symbol <- function(x)paste0("_",x,"_ravro")

# Checks whether a character value is a ravro-defined symbol value
# vectorized
is_ravro_symbol <- function(x)grepl("_[[:alnum:]_]*_ravro",x)

# Converts a ravro-defined symbol value back into the original factor level value
# vectorized
...
# Checks whether a character value is a valid Avro "Name" value
...
avro_make_data <- function(x,name=NULL,namespace=NULL,
  ...
  xname <- avro_make_type_name(xtype,name=name,namespace=namespace)
  ...
  if (xtype =="null"){
    ...
    # TODO: enlist with xname if necessary
...
# Sanitize a character value to a valid Avro "Name"
